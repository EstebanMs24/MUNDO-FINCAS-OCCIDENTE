...
import type { LoteAPI } from "@/types/lote";
...
export default function CardLote({ lote }: Props) {
  const mensaje = encodeURIComponent(
    `Hola 👋 Estoy interesado en el lote *${lote.nombre}* ubicado en *${lote.ubicacion}*, con área de *${lote.area.toLocaleString("es-CO")} m²*. ¿Podría darme más información sobre precio y condiciones de venta?`
  );
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group border border-gray-100">
      ...
      {/* Content */}
      <div className="p-5">
        <h3 className="font-bold text-lg leading-tight mb-1" style={{ color: "#333333" }}>
          {lote.nombre}
        </h3>
