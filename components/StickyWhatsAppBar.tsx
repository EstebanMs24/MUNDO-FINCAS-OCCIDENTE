...
export default function StickyWhatsAppBar({ nombreFinca, precio }: Props) {
  const [visible, setVisible] = useState(false);

  // Aparece después de hacer scroll más de 300px
  useEffect(() => {
  ...
  const mensaje = encodeURIComponent(
    `Hola 👋 Estoy interesado en la finca *${nombreFinca}*. ¿Está disponible y cuáles son las condiciones?`
  );
    ...
    >
      ...
      >
        ...
        <a
          ...
          className="flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-white text-sm transition-all active:scale-95"
