  ...
  return (
    <aside className="flex flex-col h-full">
      ...
      <nav className="flex-1 p-3 space-y-1">
        {links.map((link) => {
          const active = pathname === link.href || pathname.startsWith(link.href + "/") && link.href !== "/admin/fincas";
          return (
            <Link
              key={link.href}
              href={link.href}
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Sidebar desktop */}
      <div className="hidden lg:flex fixed top-0 left-0 h-full w-72 bg-white border-r border-gray-200 flex-col z-30">
        <Sidebar />
      </div>

      {/* Sidebar mobile drawer */}
      <div
        className={`fixed top-0 left-0 h-full w-72 bg-white border-r border-gray-200 flex flex-col z-30 transition-transform duration-300 lg:hidden ${
